 ...
 * @returns 处理后的错误信息
 */
export const handleError = (error: unknown, context = 'ProTable'): ProTableError => {
 ...
 * @returns 验证结果
 */
export const validateColumns = (columns: any[]): { valid: boolean; errors: string[] } => {
  const errors: string[] = []
...
export class ColumnBuilder {
  ...
  hideInSearch(hide = true): ColumnBuilder {
    this.column.hideInSearch = hide
    return this
  }

  hideInTable(hide = true): ColumnBuilder {
  ...
  required(required = true): ColumnBuilder {
